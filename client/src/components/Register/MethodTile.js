...
const MethodTile = ({
  ...
  const handleClick = (event) => {
    const { isAvailable: methodIsAvailable } = method;

    if (methodIsAvailable && onClick) {
      onClick(event);
    }
  };
   ...
   */
  const renderSupportLink = (methodProp) => {
    const { ss: { i18n } } = window;
    const { supportLink, supportText } = methodProp;
